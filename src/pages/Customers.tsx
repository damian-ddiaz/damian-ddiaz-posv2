...
const Customers: React.FC = () => {
    ...
    // Nuevo: Consumir el endpoint /buscar_clientes y poblar la lista de clientes
    useEffect(() => {
        const fetchClientesBackend = async () => {
            try {
                setLoading(true);
                const response = await fetch('http://localhost:3000/buscar_clientes');
                if (!response.ok) throw new Error('Error al consultar clientes');
                const data = await response.json();

                // Mapear los datos del backend al formato Client
                const clientesBackend: Client[] = data.map((item: any) => ({
                    id: item.id_cliente?.toString() ?? '',
                    razonSocial: item.nombre_razon_social ?? '',
                    registroFiscal: item.ci_rif ?? '',
                    direccionFiscal: item.direccion ?? '',
                    email: item.email ?? '',
                    telefono: item.telefono ?? '',
                    direccionEnvio: item.direccion_entrega ?? '',
                    createdAt: item.fec_reg ?? '',
                    totalCompras: 0,
                    ultimaCompra: '-',
                }));

                setCustomers(clientesBackend);
            } catch (error: any) {
                console.error('Error al consumir /buscar_clientes:', error);
                toast.error(error.message || 'Error al cargar los clientes del backend');
            } finally {
                setLoading(false);
            }
        };

        fetchClientesBackend();
    }, []);
                                    ...
                                    {currentItems.length > 0 ? (
                                        currentItems.map((customer) => (
                                            <tr key={customer.id} className="hover:bg-gray-50">
                                                <td className="px-6 py-4"> {/* BLOQUE CLIENTE */}
