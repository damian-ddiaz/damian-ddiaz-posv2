...
const Products: React.FC = () => {
    ...
    const [formData, setFormData] = useState<Omit<Product, 'id'> & { descripcion: string }>({
        code: '',
        name: '',
        descripcion: '',
        price: 0,
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                // Consumir el endpoint del backend Node.js
                const response = await fetch('http://localhost:3000/buscar_productos');
                if (!response.ok) throw new Error('Error al consultar productos');
                const data = await response.json();

                // Mapear los datos recibidos al tipo Product
    ...
    const handleOpenModal = (product?: Product) => {
        if (product) {
            ...
            setFormData({
                code: product.code,
                name: product.name,
                descripcion: (product as any).descripcion ?? '', // Usa 'as any' si Product no tiene 'descripcion'
                price: product.price,
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                />
                                                        ...
                                                        <div>
                                <label htmlFor="descrip" className="block text-sm font-medium text-gray-700">
