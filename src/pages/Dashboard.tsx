...
const Dashboard: React.FC = () => {
  ...
  const [facturasEmitidas, setFacturasEmitidas] = useState<number | null>(null);
  const [ventasdelDia, setVentasdelDia] = useState<number | null>(null);

  
  // Mock data for dashboard
  const stats = [
    { id: 1, name: 'Ventas del día', value: ventasdelDia !== null ? ventasdelDia : '...', icon: <TrendingUp className="h-6 w-6 text-primary-500" />, trend: '+12%' },
    { id: 2, name: 'Facturas emitidas', value: facturasEmitidas !== null ? facturasEmitidas : '...', icon: <Receipt className="h-6 w-6 text-success-500" />, trend: '+3' },
  ...
  useEffect(() => {
    ...
    // Obtener cantidad de facturas emitidas
    fetch('http://localhost:3000/conteo')
      .then(res => res.json())
      .then(data => {
        setFacturasEmitidas(data.total || 0);
      })
      .catch(() => setFacturasEmitidas(0));

    // Obtener Ventas del Dia - ddiaz
    fetch('http://localhost:3000/facturacion')
      .then(res => res.json())
      .then(data => {
        setVentasdelDia(data.total_ventas || 0);
      })
      .catch(() => setVentasdelDia(0));
      // Obteniendo las Ultimas 5 Facturas - ddiaz
      fetch('http://localhost:3000/ultimas_facturas')
        .then(res => res.json())
        .then(data => {
        // Si el backend retorna un solo objeto, conviértelo en array solo si no es null/undefined
        let facturas: any[] = [];
        if (Array.isArray(data)) {
          facturas = data;
        } else if (data && typeof data === 'object') {
          // Si el objeto tiene una propiedad que es un array, úsala (ajusta según tu backend)
        ...
        // Mapear los datos recibidos al formato esperado por la tabla
        const mapped = (facturas || []).map((item: any) => ({
          id: item.factura || item.id || '',
          client: item.cliente || item.client || '',
          date: item.fecha || item.date || '',
          amount: item.total !== undefined ? `$${Number(item.total).toFixed(2)}` : '',
          status: item.estado === 'PROCESADO' ? 'Pagada' : (item.estado || ''),
        }));
        // Mostrar solo las últimas 5 facturas
        setRecentInvoices(mapped.filter((inv: { id: string; client: string }) => inv.id && inv.client).slice(0, 5));
        })
